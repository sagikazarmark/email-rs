...
- **Switching transports may silently drop behavior.** This is the accepted trade, and it matches
  ...
  than relaxes, such as a sandbox flag, a suppression-list toggle, or a "never deliver to real
  recipients" switch, must not live in this bag. Silently dropping a safety control on a
  transport switch turns it into a no-op while still reporting success. Constraining controls
  belong in core `SendOptions`, where every transport must honor them or fail.
...
- **A `SendRequest` is a privileged payload.** Anyone who can reach ingress already controls
  sender, recipients, and body, so the option bag adds no meaningful escalation; however, ingress
  must be treated as an authenticated internal surface.
