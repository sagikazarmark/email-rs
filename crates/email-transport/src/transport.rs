...
pub struct SendOptions {
    ...
    /// Typed, in-process provider-specific controls. Not serialized across process
    /// boundaries; cross-process callers need their own serializable mirror.
...
///
/// Adapters define their own per-provider option structs and implement
/// this trait so the typed slot in [`TransportOptions`] can store them
/// keyed by [`TypeId`]. The trait carries no methods or associated
/// constants, provider identification flows through the typed
/// [`TypeId`] for in-process callers and through the explicit JSON-bag
/// key for queued payloads.
///
/// # Send + Sync
///
...
///
/// A typed, in-process map keyed by `TypeId` carrying provider-specific
/// strongly typed values (e.g. `PostmarkTag`, `ResendTags`). Cheap,
/// zero-copy at the adapter boundary.
///
/// The kernel does not see JSON. Cross-process callers that need to
/// serialize provider-specific overrides should hydrate them into typed
/// slots on the receiving side before the resulting `SendOptions` reaches
/// [`Transport::send`].
#[derive(Default)]
